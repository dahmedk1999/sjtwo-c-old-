...
bool uart_lab__polled_get(uart_number_e uart, char *input_byte) {
  ...
  if(uart==2){
  while(!LPC_UART2->LSR & 1);//While LSR's RDR is not set, wait until it is set (Ready)
...
bool uart_lab__polled_put(uart_number_e uart, char output_byte) {
  ...
  if(uart==2){
  while(!(LPC_UART2->LSR & 1<<5));//THR is 0 | contains valid data. 1 when empty.
  LPC_UART2->THR=output_byte;//Send data when THR is empty
  }
  else if(uart==3){
  while(!(LPC_UART3->LSR & 1<<5));//THR is 0 | contains valid data. 1 when empty.
